...
public class Sudoku {
    ...
    public int randomGenerator(int num) {
        return (int)MathUtil.floor((MathUtil.nextAfter(0.71,0.91) * num + 1));
    }
    ...
    // Print sudoku 
    public ComponentGroup printSudoku(Form parent,int N,int K) {
        
        Container myContainer = new Container(new GridLayout(9,9));
        
        ComponentGroup sudokuGroup = new ComponentGroup();
        sudokuGroup.setTactileTouch(true);
        sudokuGroup.setScrollableY(true);
        
        for (int i = 0; i < N; i++) {
            for (int j = 0; j < N; j++) {
                Button suDokuButton = new Button("" + mat[i][j] + " ");
                myContainer.add(suDokuButton);
            }
        }
        sudokuGroup.addComponent(myContainer);
        return sudokuGroup;
    }
    ...
    public static void main(String[] args) {
        ...
        if (solver.solveSudoku(sudoku.mat, N)) {
            solver.print(sudoku.mat, N); // print solution 
        } else {
